...
class DetectorManager():
    ...
    ros_image_input_topic = '/D435_head_camera/color/image_raw/' + transport
    pub_out_images = True
    ...
    def __init__(self):
        ...
        if torch.cuda.is_available():
            ...
            self.model = torch.load('model1.pt', map_location=torch.device('cpu'))
        ...
        if self.transport == "compressed":
            self.image_sub = rospy.Subscriber(self.ros_image_input_topic, ROSCompressedImage,
                                              self.__image_clbk, queue_size = 1)
            print("wating for camera message...")
            rospy.wait_for_message(self.ros_image_input_topic, ROSCompressedImage, timeout=None)
            print("... camera message arrived")
            ros_image_input = ROSCompressedImage
            
        else:
            self.image_sub = rospy.Subscriber(self.ros_image_input_topic, ROSImage,
                                              self.__image_clbk, queue_size = 10)
            print("wating for camera message...")
            rospy.wait_for_message(self.ros_image_input_topic, ROSImage, timeout=None)
            print("... camera message arrived")
            ros_image_input = ROSImage
            
        if self.pub_out_images:
            self.image_pub = rospy.Publisher("detection_output_img/compressed", ROSCompressedImage, queue_size=10)
    ...
    def infer(self):
        ...
        if self.pub_out_images:
            ...
            #cv2.imshow("test", self.cv_image_output)
            #cv2.waitKey()
